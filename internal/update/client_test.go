...
func TestSelfUpdateDownloadsAndAppliesLatestLinuxArm64Release(t *testing.T) {
	t.Parallel()

	archive := buildTarGzArchive(t, map[string][]byte{
		"chatbox": []byte("updated-linux-binary"),
	})
	archiveChecksum := sha256.Sum256(archive)
	checksums := fmt.Sprintf("%s  chatbox_linux_arm64.tar.gz\n", hex.EncodeToString(archiveChecksum[:]))
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/repos/HYPGAME/chatbox/releases/latest":
			_, _ = fmt.Fprintf(w, `{
				...
				"html_url": "%s/releases/tag/v0.2.0",
				"assets": [
					{"name": "chatbox_linux_arm64.tar.gz", "browser_download_url": "%s/assets/chatbox_linux_arm64.tar.gz"},
					{"name": "checksums.txt", "browser_download_url": "%s/assets/checksums.txt"}
				]
			}`, server.URL, server.URL, server.URL)
		case "/assets/chatbox_linux_arm64.tar.gz":
			_, _ = w.Write(archive)
		case "/assets/checksums.txt":
			_, _ = w.Write([]byte(checksums))
		default:
