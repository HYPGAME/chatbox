...
func TestSidebarDoesNotReceiveViewportOverflowWhenScrolled(t *testing.T) {
	t.Parallel()
	...
	for i := 0; i < 40; i++ {
		uiModel.addHistoryEntry(historyEntry{
			...
			from:      "host",
			body:      fmt.Sprintf("scrollable message %02d with enough words to wrap within the viewport only", i),
			at:        time.Date(2026, 5, 18, 12, i%60, 0, 0, time.Local),
		})
	}
	...
	for _, line := range strings.Split(rendered, "\n") {
		if got := lipgloss.Width(line); got > uiModel.width {
			t.Fatalf("expected scrolled sidebar view line width not to exceed terminal width %d, got %d in %q\n%s", uiModel.width, got, line, rendered)
		}
	}
}

func TestMouseDragInSidebarDoesNotScrollViewport(t *testing.T) {
	t.Parallel()
	...
	for i := 0; i < 40; i++ {
		uiModel.addHistoryEntry(historyEntry{
			...
			messageID: fmt.Sprintf("message-%02d", i),
			from:      "host",
			body:      fmt.Sprintf("scrollable message %02d", i),
			at:        time.Date(2026, 5, 18, 12, i%60, 0, 0, time.Local),
		})
	}
	uiModel.viewport.SetYOffset(10)
	initialOffset := uiModel.viewport.YOffset
	sidebarX := uiModel.width - uiModel.sidebarWidth() + 2
	...
	if got := uiModel.viewport.YOffset; got != initialOffset {
		t.Fatalf("expected sidebar drag not to scroll viewport, got offset %d want %d", got, initialOffset)
	}
}
